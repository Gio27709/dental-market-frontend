            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              ...
              <div>
                ...
                <div className="px-4 py-3 rounded-xl text-sm font-medium flex items-center gap-2" style={{ background: "#f9f9ff", color: "#6b1e96", border: "1px solid rgba(0,0,0,0.06)" }}>
                  ...
                  {user.role === 'user' ? 'Usuario' : user.role === 'professional' ? 'Profesional' : user.role === 'student' ? 'Estudiante 🎓' : user.role === 'admin' ? 'Administrador' : user.role === 'owner' ? 'Owner 👑' : (user.role === 'store' || user.role === 'store/owner') ? 'Tienda' : user.role}
                </div>
              </div>
            </div>
          </div>
