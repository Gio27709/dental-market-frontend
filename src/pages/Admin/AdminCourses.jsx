...
export default function AdminCourses() {
  ...
  const executeDelete = async () => {
    ...
    try {
      ...
      toast.error(err.response?.data?.error || "Error al eliminar curso");
    } finally {
      setDeleting(false);
    }
  };
  ...
  return (
    <div className="max-w-5xl">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-gray-900">Gestión de Cursos</h1>
          <p className="text-sm text-gray-500 mt-1">
            Administra los cursos, estudios y videos educativos de la plataforma.
          </p>
        </div>
        <button
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-12 text-center shadow-sm">
           ...
           <p className="text-sm text-gray-500 mb-6">Comienza agregar contenido educativo al sistema.</p>
           <button onClick={openCreateModal} className="px-5 py-2.5 bg-[#531575] text-white font-semibold rounded-xl text-sm">Registrar Curso</button>
        </div>
      ) : (
                ...
                <tr key={course.id} className="hover:bg-gray-50/50 transition-colors">
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="max-w-[200px]">
                        <p className="font-semibold text-gray-900 truncate" title={course.title}>{course.title}</p>
                        <p className="text-xs text-gray-400">Inst: {course.instructor}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-5 py-3.5 text-center">
                    <span className="inline-flex px-2 py-1 text-[10px] font-bold uppercase tracking-wider bg-purple-50 text-purple-700 rounded-md">{course.level || 'Todos'}</span>
                  </td>
                  <td className="px-5 py-3.5 text-center">
                  ...
                  <td className="px-5 py-3.5 text-center">
                     {course.is_published ? (
                       ...
                       <span className="inline-flex items-center gap-1 text-xs font-semibold text-gray-500 bg-gray-100 px-2 py-1 rounded border border-gray-200">
                  ...
                  <td className="px-5 py-3.5 text-right">
                    <div className="flex items-center justify-end gap-2">
                       <button onClick={() => openEditModal(course)} className="p-2 rounded-lg text-gray-400 hover:text-[#531575] hover:bg-[#531575]/10 transition-colors">
                          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-4 h-4"><path strokeLinecap="round" strokeLinejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" /></svg>
                       </button>
                       <button onClick={() => confirmDelete(course)} className="p-2 rounded-lg text-gray-400 hover:text-red-600 hover:bg-red-50 transition-colors">
      ...
      {/* Course Modal */}
      {modalOpen && (
        <CourseModal course={editingCourse} onSave={handleSave} onClose={closeModal} />
      )}
        ...
        <form onSubmit={handleSubmit} className="p-6 overflow-y-auto flex-1 space-y-5">
           ...
           <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                 ...
                 <input type="text" value={form.category} onChange={e => setForm({...form, category: e.target.value})} className="w-full px-4 py-2 border rounded-xl" placeholder="Ej: Ortodoncia" />
              </div>
           </div>
           ...
           <div className="grid grid-cols-2 gap-4">
              <div>
                 ...
                 <select value={form.level} onChange={e => setForm({...form, level: e.target.value})} className="w-full px-4 py-2 border rounded-xl">
                   ...
                   <option value="All Levels">Para todos</option>
                 </select>
              </div>
              <div>
                 <label className="block text-sm font-semibold text-gray-700 mb-1">Precio (USD) - 0 es Gratis</label>
                 <input type="number" min="0" step="0.01" value={form.price} onChange={e => setForm({...form, price: parseFloat(e.target.value) || 0})} className="w-full px-4 py-2 border rounded-xl" />
              </div>
           </div>

           <div>
             <label className="block text-sm font-semibold text-gray-700 mb-1">Enlace del Video (YouTube Enbed o URL externa) *</label>
             <input required type="text" value={form.video_url} onChange={e => setForm({...form, video_url: e.target.value})} className="w-full px-4 py-2 border rounded-xl" placeholder="https://www.youtube.com/embed/..." />
           </div>

           <div>
             <label className="block text-sm font-semibold text-gray-700 mb-1">URL de Portada (Thumbnail)</label>
           ...
           <div>
             <label className="block text-sm font-semibold text-gray-700 mb-1">Descripción (Soporta HTML básico)</label>
             <textarea rows={4} value={form.description} onChange={e => setForm({...form, description: e.target.value})} className="w-full px-4 py-2 border rounded-xl" placeholder="Escribe el contenido del curso aquí..." />
           </div>
           ...
           <div className="flex items-center gap-2">
             ...
             <label htmlFor="publicado" className="text-sm font-semibold text-gray-800 cursor-pointer">Activo y Público (Visible para usuarios)</label>
           </div>
  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl w-full max-w-sm p-6 text-center shadow-xl">
        ...
        <h3 className="text-lg font-bold text-gray-900 mb-2">¿Eliminar curso?</h3>
        <p className="text-sm text-gray-500 mb-6">Esta acción es irreversible y eliminará todo el contenido asociado al curso.</p>
        <div className="flex items-center gap-3">
