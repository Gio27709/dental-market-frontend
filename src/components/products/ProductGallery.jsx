...
export default function ProductGallery({ images = [] }) {
  ...
  return (
    <div className="flex flex-col sm:flex-row gap-4">
        ...
        <div className="hidden sm:flex flex-col gap-2.5 overflow-y-auto max-h-[520px] w-[90px] flex-shrink-0 pr-1">
          {images.map((img, idx) => (
            <button
              ...
              className={`relative h-[90px] w-[90px] flex-shrink-0 rounded-md overflow-hidden border-2 transition-all duration-200 bg-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#6b1e96] focus-visible:ring-offset-2 ${
                currentIndex === idx
        ...
        {images.length > 1 && (
          <>
            <button
              ...
              className="absolute left-3 top-1/2 -translate-y-1/2 w-10 h-10 bg-white hover:bg-gray-50 text-gray-500 hover:text-[#6b1e96] rounded-full shadow-md border border-gray-200 flex items-center justify-center transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#6b1e96] focus-visible:ring-offset-2"
            >
            ...
            <button
              ...
              className="absolute right-3 top-1/2 -translate-y-1/2 w-10 h-10 bg-white hover:bg-gray-50 text-gray-500 hover:text-[#6b1e96] rounded-full shadow-md border border-gray-200 flex items-center justify-center transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#6b1e96] focus-visible:ring-offset-2"
            >
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-5 h-5">
      ...
      {images.length > 1 && (
        <div className="sm:hidden flex gap-2 overflow-x-auto p-1">
          {images.map((img, idx) => (
            <button
              key={idx}
              type="button"
              ...
              aria-label={`Ver imagen ${idx + 1} de ${images.length}`}
              aria-current={currentIndex === idx ? "true" : undefined}
              className={`relative h-16 w-16 flex-shrink-0 rounded-md overflow-hidden border-2 transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#6b1e96] focus-visible:ring-offset-2 ${
                currentIndex === idx
