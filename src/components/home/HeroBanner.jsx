    ...
    <div className="flex flex-col gap-6 w-full mb-12">
      ...
      <section className="relative h-[200px] sm:h-[250px] lg:h-[300px] w-full rounded-2xl overflow-hidden shadow-xl bg-gray-900 group">
        ...
        <div className="relative h-full flex flex-col justify-center px-8 lg:px-12 max-w-3xl text-white">
          ...
          <h1 className="text-2xl sm:text-3xl lg:text-5xl font-black leading-[1.1] mb-3 tracking-tight whitespace-pre-wrap">
            {heading}{" "}
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {promoCards.map((card, idx) => {
          ...
          return (
            <div key={idx} className={`bg-white p-6 rounded-2xl border border-gray-200 shadow-sm flex items-center justify-between group hover:border-primary-500 transition-all ${idx === 2 ? 'md:col-span-2 lg:col-span-1' : ''}`}>
              <div className="flex-1">
                <span className={`${badgeColorClass} font-bold text-xs uppercase tracking-wider`}>
                  {card.badge}
                </span>
                <h3 className="text-base font-bold mt-1 text-gray-900">
                  {card.title}
                </h3>
                <p className="text-gray-500 text-xs mb-3">{card.description}</p>
                <Link
                  to={card.link_url || "#"}
                  className={`${linkColorClass} text-sm font-bold flex items-center gap-1 group-hover:underline`}
                >
                  {card.link_text}{" "}
