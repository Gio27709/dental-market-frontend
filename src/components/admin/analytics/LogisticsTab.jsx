    ...
    <div className="space-y-6">
      ...
      {(slaBlocked || (sla.unmeasurableDispatched ?? 0) > 0 || (sla.openPastDue ?? 0) > 0) && (
        <div
          className={`rounded-2xl p-4 text-xs text-fx-muted leading-relaxed space-y-2 ${
            slaBlocked ? "bg-fx-neg/5 border border-fx-neg/30" : "bg-fx-warn/5 border border-fx-warn/30"
          }`}
        >
          {slaBlocked && (
            <p>
              <span className="font-semibold text-fx-neg uppercase tracking-wide">SLA retenido · </span>
          ...
          {(sla.unmeasurableDispatched ?? 0) > 0 && (
            <p>
              <span className="font-semibold text-fx-warn uppercase tracking-wide">Cobertura · </span>
          ...
          {(sla.openPastDue ?? 0) > 0 && (
            <p>
              <span className="font-semibold text-fx-warn uppercase tracking-wide">Vencidos sin despachar · </span>
        ...
        <ResponsiveContainer width="100%" height={260}>
          {chartMode === "bar" ? (
            <BarChart data={deliveryTrend}>
              ...
              <Bar dataKey="delivered_count" name="Entregados" fill="#6b1e96" radius={[4, 4, 0, 0]} />
              <Bar dataKey="failed_count" name="Fallidos" fill="#b8482f" radius={[4, 4, 0, 0]} />
            </BarChart>
          ) : chartMode === "line" ? (
            <LineChart data={deliveryTrend}>
              ...
              <YAxis stroke="#877f92" fontSize={11} />
              <Tooltip contentStyle={{ backgroundColor: "#f7f4fc", border: "1px solid #00000020", borderRadius: "10px", color: "#33243d", fontSize: 12, boxShadow: "0 8px 24px rgba(0,0,0,0.55)" }} />
              <Line type="monotone" dataKey="delivered_count" name="Entregados" stroke="#6b1e96" strokeWidth={3} dot={false} />
              <Line type="monotone" dataKey="failed_count" name="Fallidos" stroke="#b8482f" strokeWidth={2} dot={false} />
            </LineChart>
          ) : (
            <AreaChart data={deliveryTrend}>
              ...
              <YAxis stroke="#877f92" fontSize={11} />
              <Tooltip contentStyle={{ backgroundColor: "#f7f4fc", border: "1px solid #00000020", borderRadius: "10px", color: "#33243d", fontSize: 12, boxShadow: "0 8px 24px rgba(0,0,0,0.55)" }} />
              <Area type="monotone" dataKey="delivered_count" name="Entregados" stroke="#6b1e96" strokeWidth={3} fillOpacity={1} fill="url(#colorDelivered)" />
            </AreaChart>
          )}
        </ResponsiveContainer>
      </ChartCard>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <DataTable
          ...
          columns={[
            ...
            { header: "Estado", accessor: "is_active", render: (r) => <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${r.is_active ? "bg-fx-pos/10 text-fx-pos" : "bg-fx-neg/10 text-fx-neg"}`}>{r.is_active ? "ACTIVO" : "INACTIVO"}</span> }
          ]}
          data={riders}
        />
        <DataTable
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <DataTable
          ...
          columns={[
            { header: "Motivo de Falla / Cancelación", accessor: "reason", render: (r) => <span className="font-semibold text-fx-neg">{r.reason}</span> },
